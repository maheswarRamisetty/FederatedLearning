...
@app.route("/xai-features")
def xai():
    return render_template("xai.html",fl_status=fl_status)
...
def trigger_client_execution():
    client_threads = []
    clients = {
        "Client 1": ["python", "c1.py"],
        "Client 2": ["python", "c2.py"],
        "Client 3": ["python", "c3.py"],
    }
...
def load_model():
    try:
        with open("./xai/logistic_regression_model.pkl", "rb") as f:
            model = joblib.load(f)
